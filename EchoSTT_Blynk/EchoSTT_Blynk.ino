...
String Auth_Token = "BLYNK_AUTH_TOKEN";
String Pin_Number = "0";
String Pin_Status = "0";
String Server = "http://188.166.206.43/" + Auth_Token + "/update/" ;
...
void setup()
{
  ...
  rest.settoken("YOUR_ATOM_ECHO_TOKEN");//<------------------------------------REPLACE YOUR_TOKEN-------------------
...
void loop()
{
  ...
  if (M5.Btn.isPressed())
  {
    ...
    if (rest.Pcm2String(microphonedata0, data_offset, DEV_PID_ENGLISH, &SpakeStr) != -1)
    {
      ...
      if (SpakeStr == "turn on lamp")
      {
        Serial.println("Turning on the Lamp");
        Pin_Number = "V1";
        Pin_Status = "1";
        Sending_Request_To_Blynk(Pin_Number, Pin_Status);
      }
      if (SpakeStr == "turn off lamp")
      {
...
void Sending_Request_To_Blynk(String Pin_Number, String Pin_Status)
{
  HTTPClient http;

  Serial.println("Sending Request to Blynk Server");
  String Request_Link = Server + Pin_Number + "?value=" + Pin_Status ; //188.166.206.43/Auth_Token/update/Pin_Number?value=Pin_Status 

  http.begin(Request_Link); //HTTP
